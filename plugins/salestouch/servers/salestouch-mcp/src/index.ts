...
type ToolRisk = "read" | "mutate" | "approval";
...
const fallbackResources: ResourceCatalogEntry[] = [
  {
...
async function fetchCatalog() {
  const payload = await apiRequest<{ data?: CatalogPayload }>("/api/v1/commands");
...
function deriveCategory(commandId: string) {
  if (commandId.startsWith("lead.")) return "leads";
  if (commandId.startsWith("mission.")) return "missions";
  if (commandId.startsWith("offer.")) return "offers";
  if (commandId.startsWith("import.")) return "imports";
  if (commandId.startsWith("scoring.")) return "scoring";
  return "linkedin";
}

async function fetchCommandSchema(commandId: string) {
  const cached = schemaCache.get(commandId);
  if (cached) {
    return cached;
  }

  const payload = await apiRequest<{ data?: CommandSchemaPayload }>(
    `/api/v1/commands/${encodeURIComponent(commandId)}/schema`,
  );
...
async function fetchResources() {
  const payload = await apiRequest<{ data?: ResourceListPayload }>("/api/v1/resources");
  return payload.data?.resources ?? fallbackResources;
}

async function readResource(uri: string) {
  const payload = await apiRequest<{ data?: ResourceReadPayload }>(
...
function getToolAnnotations(risk: ToolRisk) {
  if (risk === "read") {
    return {
      readOnlyHint: true,
...
async function handleInitialize(id: JsonRpcId, params?: Record<string, unknown>) {
  const requestedVersion =
    typeof params?.protocolVersion === "string" ? params.protocolVersion : null;
  const protocolVersion =
    requestedVersion && PROTOCOL_VERSIONS.includes(requestedVersion)
      ? requestedVersion
      : PROTOCOL_VERSIONS[0];
...
async function handleToolCall(id: JsonRpcId, params?: Record<string, unknown>) {
  ...
  }>(`/api/v1/commands/${encodeURIComponent(toolName)}`, {
    method: "POST",
    body: JSON.stringify({
...
async function handleResourcesList(id: JsonRpcId) {
  const resources = await fetchResources();
  sendResult(id, { resources });
}

async function handleResourceRead(id: JsonRpcId, params?: Record<string, unknown>) {
...
function processInputBuffer() {
  while (true) {
    ...
    try {
      const request = JSON.parse(messageBody) as JsonRpcRequest;
