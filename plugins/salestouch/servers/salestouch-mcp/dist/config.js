...
function getLegacyConfigPath(configPath) {
    return path.join(path.dirname(configPath), LEGACY_CONFIG_FILE_NAME);
}
async function findExistingProjectConfigBaseDir(startDir) {
...
async function findNearestProjectBaseDir(startDir) {
...
export async function getProjectConfigPath(startDir) {
...
function getResolutionStartDir() {
    return (readEnvString("SALESTOUCH_PROJECT_DIR") ||
        readEnvString("INIT_CWD") ||
        process.cwd());
}
export async function resolvePluginApiConfig(options) {
    ...
    const accessToken = envAccessToken || projectConfig.accessToken || globalConfig.accessToken || null;
    ...
    if (!apiKey && !accessToken) {
        throw new Error("Missing SalesTouch authentication. Run `salestouch setup`, `salestouch auth login`, or provide SALESTOUCH_API_KEY / SALESTOUCH_ACCESS_TOKEN.");
    }
    return {
        baseUrl,
        apiKey,
